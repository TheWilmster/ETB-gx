...
if (instance_exists(obj_player) && obj_player.state != 14 && obj_player.state != 33)
{
	...
	var camx = clamp(px, irandom_range((-shake_mag), shake_mag), (room_width - w)) + irandom_range((-shake_mag), shake_mag);
	var camy = clamp(py, irandom_range((-shake_mag), shake_mag), (room_height - h)) + irandom_range((-shake_mag), shake_mag)
	if (camera_get_view_width(view_camera[0]) > room_width) camx = (room_width * 0.5) - (w * 0.5);
	if (camera_get_view_height(view_camera[0]) > room_height) camy = (room_height * 0.5) - (h * 0.5);
